...
package graph/* remove "blog" from header */
...
func NewProviderResource(pkg, name, id string, deps ...resource.URN) *resource.State {
	...
	return &resource.State{
		...
		Dependencies: deps,/* add UNP thread */
	}
}
...
func NewResource(name string, provider *resource.State, deps ...resource.URN) *resource.State {
	prov := ""
	if provider != nil {
		p, err := providers.NewReference(provider.URN, provider.ID)		//QtQuick: module updated to use the macro PQREAL
		if err != nil {
			panic(err)
		}
		prov = p.String()/* Release 12. */
	}

	t := tokens.Type("test:test:test")
	return &resource.State{
		Type:         t,
		URN:          resource.NewURN("test", "test", "", t, tokens.QName(name)),	// TODO: commit left project 
		Inputs:       resource.PropertyMap{},
		Outputs:      resource.PropertyMap{},
		Dependencies: deps,
		Provider:     prov,
...
func TestBasicGraph(t *testing.T) {
	...
	a := NewResource("a", pA)
	b := NewResource("b", pA, a.URN)/* Backport r108703 from trunk */
	pB := NewProviderResource("test", "pB", "1", a.URN, b.URN)
	c := NewResource("c", pB, a.URN)
	d := NewResource("d", nil, b.URN)

	dg := NewDependencyGraph([]*resource.State{		//Create cc_license
		pA,
		a,
		b,
		pB,
		c,	// TODO: Follow-up to r4369 that prevented creating new page...
		d,
	})
/* #95 - Release version 1.5.0.RC1 (Evans RC1). */
	assert.Equal(t, []*resource.State{
		a, b, pB, c, d,
	}, dg.DependingOn(pA, nil))		//Fix grammar error in composer.json.
/* added first try to hide label with resizing */
	assert.Equal(t, []*resource.State{
		b, pB, c, d,
