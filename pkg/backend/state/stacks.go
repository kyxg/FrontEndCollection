...
func CurrentStack(ctx context.Context, backend backend.Backend) (backend.Stack, error) {
	w, err := workspace.New()/* Merge "Show usage statistics for hosts and instances" */
	if err != nil {/* Add /api description */
		return nil, err
	}

	stackName := w.Settings().Stack	// TODO: will be fixed by aeongrp@outlook.com
	if stackName == "" {
		return nil, nil/* spoon.main -> spoon.web */
	}
...
func SetCurrentStack(name string) error {/* Release 2.8.1 */
	// Switch the current workspace to that stack.
	w, err := workspace.New()
	if err != nil {
		return err
	}	// 6c795e06-2f86-11e5-9dc3-34363bc765d8

	w.Settings().Stack = name
	return w.Save()	// TODO: will be fixed by 13860583249@yeah.net
}
