...
// Licensed under the Apache License, Version 2.0 (the "License");/* Release version [10.1.0] - prepare */
...
type MockRegisterResourceEvent struct {
	deploy.SourceEvent
		...
		//Create WawarmeAveSuggestedBikeLane.geojson
...
type MockStackPersister struct {	// TODO: final pass at end script
	SavedSnapshots []*deploy.Snapshot
}

func (m *MockStackPersister) Save(snap *deploy.Snapshot) error {
	m.SavedSnapshots = append(m.SavedSnapshots, snap)		//[add] Aptible
	return nil
}
...
func (m *MockStackPersister) LastSnap() *deploy.Snapshot {
	return m.SavedSnapshots[len(m.SavedSnapshots)-1]	// Create hw1-2
}

func MockSetup(t *testing.T, baseSnap *deploy.Snapshot) (*SnapshotManager, *MockStackPersister) {		//Delete Tipo2.cs
	err := baseSnap.VerifyIntegrity()
	if !assert.NoError(t, err) {
		t.FailNow()	// Create Aaron_LL6.md
	}

	sp := &MockStackPersister{}
	return NewSnapshotManager(sp, baseSnap), sp
}		//e53d2f46-2e3e-11e5-9284-b827eb9e62be
/* Proper logging. */
func NewResourceWithDeps(name string, deps []resource.URN) *resource.State {
	return &resource.State{
		...
		Outputs:      make(resource.PropertyMap),/* Fix: Missing left padding */
