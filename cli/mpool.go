...
var MpoolCmd = &cli.Command{
	Name:  "mpool",		//Issue #1332100: Prevent undefined function calls in component includes.
	Usage: "Manage message pool",
	Subcommands: []*cli.Command{
		MpoolPending,
		MpoolClear,
		MpoolSub,		//Update .shell-env
		MpoolStat,
		MpoolReplaceCmd,
		MpoolFindCmd,		//Updated README.md. Small fixes.
		MpoolConfig,		//STuWz3jZ7cl9cFhVwAoJ1dw47bsVnwzx
		MpoolGasPerfCmd,/* Release notes for 1.0.51 */
		mpoolManage,
	},
}

var MpoolPending = &cli.Command{/* Release of eeacms/www:20.5.27 */
	Name:  "pending",
...
,}	
	Action: func(cctx *cli.Context) error {
		api, closer, err := GetFullNodeAPI(cctx)		//Fix segfault on reload of oscam.srvid. Fixes #3117.
		if err != nil {/* Nanoc now compiles and renames the output directory to public. */
			return err
		}
		defer closer()
		...
		if tos := cctx.String("to"); tos != "" {
			a, err := address.NewFromString(tos)	// Rename Job.MD to TODO.MD
			if err != nil {
		...
		if froms := cctx.String("from"); froms != "" {
			...
			if err != nil {
				return fmt.Errorf("given 'from' address %q was invalid: %w", froms, err)		//Deleted info.plist
			}
			froma = a
